...
impl Program {
    ...
    pub fn add_field(&mut self, name: String, val: Value) {
        self.fields.insert(name, val);
    }
    /// Initializes all the fields in the program and stores them in the host.
    /// # Errors
    /// fails when the `Program` cannot be saved
    pub fn publish(self) -> Result<ProgramContext, Error> {
        // get the program_id from the host
        let ctx = init_program_storage();
...
impl Store for Value {
    ...
    fn as_bytes(&self) -> Cow<'_, [u8]> {
        match self {
            Value::StringObject(val) => Cow::Borrowed(val.as_bytes()),
            Value::MapObject => Cow::Borrowed(&[]),
            // hypersdk's codec.Packer uses big endian
            Value::IntObject(val) => Cow::Owned(val.to_be_bytes().to_vec()),
            Value::AddressObject(val) => {
                let bytes: &[u8] = val.as_bytes();
                Cow::Borrowed(bytes)
            }
            Value::ProgramObject(val) => {
        ...
        Self: Sized,
    {
        ...
        match tag.0 {
            1 => match to_string(bytes.to_vec()) {
                Ok(val) => Ok(Value::StringObject(val)),
                Err(_) => Err(StorageError::InvalidBytes),
            },
            2 => Ok(Value::MapObject),
            3 => {
                let num = int_from_bytes(bytes)?;
                Ok(Value::IntObject(num))
            }
            4 => {
                ...
                Ok(Value::AddressObject(Address::new(address_bytes)))
            }
            5 => {
                let num = int_from_bytes(bytes)?;
                Ok(Value::ProgramObject(ProgramContext::from(num)))
            }
    ...
    fn as_tag(&self) -> Tag {
        match self {
            Value::StringObject(_) => Tag(1),
            Value::MapObject => Tag(2),
            Value::IntObject(_) => Tag(3),
            Value::AddressObject(_) => Tag(4),
            Value::ProgramObject(_) => Tag(5),
        }
    }
}
